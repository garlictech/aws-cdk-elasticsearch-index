...
describe('Elasticsearch Index Custom Resource Stack', () => {
  it('Creates On Event Handler', async () => {
    ...
    if (!existsSync(path.join(__dirname, '..', '..', 'dist', 'src', 'on-event', 'on-event.js'))) {
      await new Promise((resolve, reject) => {
        exec('npm run webpack', {cwd: path.join(__dirname, '..', '..')}, (error) => {
    ...
    // THEN
    expectCDK(stack).to(haveResource('AWS::Lambda::Function',{Handler: 'on-event.handler'}));
  });
});
